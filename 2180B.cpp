...
void solve() {
    ...
    // Initialize s with the first string
    string s = a[0];

    // For subsequent strings, decide whether to put them at the front or back
    for (int i = 1; i < n; ++i) {
        string front = a[i] + s;
        string back = s + a[i];
        
        // Greedily choose the lexicographically smaller result
        if (front < back) {
            s = front;
        } else {
            s = back;
