...
// ------------------------------ Main Function ------------------------------------------
int N;                 // <-- declaration (must come BEFORE the functions)
vector<int> fen;       // <-- declaration

void fadd(int i)   { for(; i <= N; i += i & -i) fen[i]++; }
int  fquery(int i) { int s = 0; for(; i > 0; i -= i & -i) s += fen[i]; return s; }

void solve() {
    int k;
    cin >> N >> k;
    vector<int> freq(N + 1, 0);          // 'freq', NOT 'cnt'
    ...
    for (int v = N; v >= 1; v--) {
        if (!freq[v]) continue;
        int hi = min(N, v + k);
        bool jumpWin  = (fquery(hi) - fquery(v)) > 0;
        bool landLose = !jumpWin && (freq[v] & 1);
        if (!landLose) yes = true;
        if (landLose) fadd(v);
    }
    cout << (yes ? "YES" : "NO") << '\n';
}
